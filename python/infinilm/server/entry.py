...
CUDA-graph policy (``--cudagraph-policy`` / ``INFINI_CUDAGRAPH_POLICY``):
  ...
  full_and_piecewise — vLLM dual-mode: FULL uniform decode + PIECEWISE
                       ...
                       PIECEWISE exists). MetaX: FA ``INFINI_FA_INGRAPH=decode``
                       (FULL segs≈1; kill-switch ``0``); MoE in-graph via
                       ``INFINI_MOE_INGRAPH``; ``INFINI_MOE_FORCE_HOST_BREAK``
                       bisect escape.
...
def _peel_entry_args(argv: Optional[Sequence[str]] = None):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "PIECEWISE homogeneous prefill (MIXED→eager); MetaX FA_INGRAPH=decode "
            "(FULL segs≈1) + MoE phase-scoped. Kill-switch FA=0; "
            "MOE_FORCE_HOST_BREAK bisect. Do not pass track_b."
        ),
    )
    parser.add_argument(
        "-h",
    ...
    try:
        ...
        if entry_args.help or "-h" in remaining or "--help" in remaining:
            print(
                ...
                "                      MIXED→eager; MetaX FA_INGRAPH=decode +\n"
                "                      MoE phase-scoped).\n"
                "                      Sets INFINI_CUDAGRAPH_POLICY; CLI wins.\n"
                "                      FA kill-switch=0; MOE_FORCE_HOST_BREAK bisect.\n"
        ...
        logger.info(
            ...
            "INFINI_NATIVE_CG_CAPTURE_BUCKETS=%s INFINI_FA_INGRAPH=%s "
            "INFINI_MOE_FORCE_HOST_BREAK=%s",
            applied,
            ...
            os.environ.get("INFINI_FA_INGRAPH", "off"),
            os.environ.get("INFINI_MOE_FORCE_HOST_BREAK", "0"),
        )
