...
void Attention::forward_pre_attn_rope_piecewise(const infinicore::Tensor &position_ids,
    ...
    const size_t valid_len = piecewise.valid_seq_len > 0 ? piecewise.valid_seq_len : seq_len;
    // Under recording / stream capture, do not allocate or rebind addresses:
    // prefer views/narrows of address-stable graph_input.position_ids and
    // staging Q/K. Past/pad length is carried by buffer *contents* after
    // copy_runtime_into_bucket_ (topology stays fixed-bucket).
    ...
    if (pos_shape.size() == 2) {
        auto pos_narrowed = position_ids->narrow({{0, 0, 1}});
        if (addr_stable) {
            // View without contiguous — bank is row0 of [1, bucket] or equivalent.
        ...
        if (addr_stable) {
            if (!narrowed->is_contiguous()) {
                throw std::runtime_error(
                    "Attention::forward_pre_attn_rope_piecewise: position_ids narrow "
                    "not contiguous under graph recording (would rebind addresses)");
            }
    ...
    if (!q_rope->is_contiguous()) {
        if (addr_stable) {
            throw std::runtime_error(
                "Attention::forward_pre_attn_rope_piecewise: q_rope not contiguous "
                "under graph recording (would rebind addresses)");
        }
    ...
    if (!k_rope->is_contiguous()) {
        if (addr_stable) {
            throw std::runtime_error(
                "Attention::forward_pre_attn_rope_piecewise: k_rope not contiguous "
                "under graph recording (would rebind addresses)");
        }
